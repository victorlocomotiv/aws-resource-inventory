                ...
                try:
                    if 'detail_method' in config:
                        ...
                        item = self._get_items_from_response(
                            getattr(client, detail['name'])(**params),
                            detail['response_key']
                        )[0]
    ...
    def collect_resources(self):
        ...
        for service_name, config in self.config['resources'].items():
            ...
            if config['service'] == 's3':
                resources = self._get_resource_data(service_name, 'us-east-1', config)
                all_resources.extend(resources)
            else:
                for region in self.regions:
                    resources = self._get_resource_data(service_name, region, config)
                    all_resources.extend(resources)
